...
public class ImageUtils {
     ...
     * @throws IOException
     */
    public static InputStream reSize(BufferedImage image, int width, String imageFormat) throws IOException {
        ...
        if(iwidth<width)
            ...
            float r = iwidth/width;
            height = iheight / r;
        }
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        InputStream in = getInputStream(image,imageFormat);
        Thumbnailator.createThumbnail(in, os, width, (int)height);
        in.close();
        os.close();
        InputStream is = new ByteArrayInputStream(os.toByteArray());
        return is;
    ...
    public static InputStream clipXCenter(BufferedImage image, String imageType, int x, int y) throws IOException {
        ...
        if(height/width>0.6) {
            ...
            return createThumbnails(is, 350, 210);
        }else{
            return createThumbnails(getInputStream(image, imageType), 350, 210);
        }
    }
